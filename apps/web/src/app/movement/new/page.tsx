...
function CreateMovementPageSkeleton() {
  return (
...
function CreateMovementForm() {
  ...
  const isLoading = trackingTypesLoading || muscleGroupsLoading;
  ...
  const onSubmit = handleSubmit(async (values: FormData) => {
    try {
      ...
      // Navigate to the return location or the new movement
      if (returnTo.includes("[movementId]")) {
        router.push(returnTo.replace("[movementId]", newMovement.id));
      } else {
  ...
  const handleCancel = () => {
    router.push(returnTo.includes("[movementId]") ? "/library" : returnTo);
  };
  ...
  if (isLoading) {
    return <CreateMovementPageSkeleton />;
  }
    ...
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Leave empty to use the default timer
        </p>
      </div>
  ...
  return (
    <ProtectedRoute>
      <div className="min-h-screen bg-background">
        <ContextualNavigation context={{ type: "create-movement" }} />
        <main className="p-2 sm:p-4 lg:p-6">
          <div className="max-w-4xl mx-auto space-y-2 sm:space-y-4 mt-2">
            <Typography variant="title1">Create custom movement</Typography>

            <Card>
              <CardContent className="p-4 sm:p-6">
                <Suspense fallback={<CreateMovementPageSkeleton />}>
                  <CreateMovementForm />
                </Suspense>
              </CardContent>
            </Card>
