...
const meta = {
  ...
  beforeEach: async () => {
    ...
    await import('@/lib/supabase/auth-utils').then((module) => {
      // @ts-ignore
...
export const ActualLoginPage: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const container = canvas.getByRole('main') || emailInput.closest('.min-h-screen');
...
export const RealFormValidation: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(async () => {
      ...
      const validationMessages = canvas.queryAllByText(/required|invalid|enter/i);
...
export const RealFormInteraction: Story = {
  name: 'Real Form Input Interaction',
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    ...
    const passwordToggle = canvas.queryByRole('button', { name: /toggle password visibility|show password|hide password/i });
...
export const RealEmailValidation: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(async () => {
      const emailError = canvas.queryByText(/valid email|email format|invalid email/i);
...
export const RealNavigationLinks: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const registerLink = canvas.queryByRole('link', { name: /sign up|register|create account/i });
    const forgotPasswordLink = canvas.queryByRole('link', { name: /forgot password|reset password/i });
...
export const RealResponsiveLayout: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByRole('textbox', { name: /email/i })).toBeInTheDocument();
