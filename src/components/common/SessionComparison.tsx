 ...
 * ];
 * const mockMovement = { tracking_type: 'weight' | 'bodyweight' | 'duration' | 'distance' | 'reps_only', ... };
 */
...
export default function SessionComparison({
  ...
  const calculateMetrics = (): MetricData[] => {
    ...
    const setsPercent = previousSetCount > 0 ? (setsDiff / previousSetCount) * 100 : 0;
    ...
    switch (movement.tracking_type) {
      case 'weight':
        // Total reps
        const currentReps = currentSets.reduce((sum, set) => sum + (set.reps || 0), 0);
        const previousReps = previousSets.reduce((sum, set) => sum + (set.reps || 0), 0);
        const repsDiff = currentReps - previousReps;
        const repsPercent = previousReps > 0 ? (repsDiff / previousReps) * 100 : 0;

        // Total volume (reps × weight)
        const currentVolume = currentSets.reduce((sum, set) => 
        ...
        const volumePercent = previousVolume > 0 ? (volumeDiff / previousVolume) * 100 : 0;

        // Weight per rep (total weight / total reps)
        const currentWeightPerRep = currentReps > 0 ? 
          currentSets.reduce((sum, set) => sum + (set.weight || 0), 0) / currentReps : 0;
        const previousWeightPerRep = previousReps > 0 ? 
          previousSets.reduce((sum, set) => sum + (set.weight || 0), 0) / previousReps : 0;
        const weightPerRepDiff = currentWeightPerRep - previousWeightPerRep;
        const weightPerRepPercent = previousWeightPerRep > 0 ? 
          (weightPerRepDiff / previousWeightPerRep) * 100 : 0;
      ...
      case 'reps_only':
        // Total reps
        const currentBodyweightReps = currentSets.reduce((sum, set) => sum + (set.reps || 0), 0);
        const previousBodyweightReps = previousSets.reduce((sum, set) => sum + (set.reps || 0), 0);
        const bodyweightRepsDiff = currentBodyweightReps - previousBodyweightReps;
        const bodyweightRepsPercent = previousBodyweightReps > 0 ? (bodyweightRepsDiff / previousBodyweightReps) * 100 : 0;
      ...
      case 'duration':
        // Total time
        const currentTotalTime = currentSets.reduce((sum, set) => sum + (set.duration || 0), 0);
        const previousTotalTime = previousSets.reduce((sum, set) => sum + (set.duration || 0), 0);
        const totalTimeDiff = currentTotalTime - previousTotalTime;
        const totalTimePercent = previousTotalTime > 0 ? (totalTimeDiff / previousTotalTime) * 100 : 0;

        // Average time per set
        const currentAvgTime = currentSetCount > 0 ? currentTotalTime / currentSetCount : 0;
        const previousAvgTime = previousSetCount > 0 ? previousTotalTime / previousSetCount : 0;
        const avgTimeDiff = currentAvgTime - previousAvgTime;
        const avgTimePercent = previousAvgTime > 0 ? (avgTimeDiff / previousAvgTime) * 100 : 0;
      ...
      case 'distance':
        // Total distance
        const currentTotalDistance = currentSets.reduce((sum, set) => sum + (set.distance || 0), 0);
        const previousTotalDistance = previousSets.reduce((sum, set) => sum + (set.distance || 0), 0);
        const totalDistanceDiff = currentTotalDistance - previousTotalDistance;
        const totalDistancePercent = previousTotalDistance > 0 ? (totalDistanceDiff / previousTotalDistance) * 100 : 0;

        // Average distance per set
        const currentAvgDistance = currentSetCount > 0 ? currentTotalDistance / currentSetCount : 0;
        const previousAvgDistance = previousSetCount > 0 ? previousTotalDistance / previousSetCount : 0;
        const avgDistanceDiff = currentAvgDistance - previousAvgDistance;
        const avgDistancePercent = previousAvgDistance > 0 ? (avgDistanceDiff / previousAvgDistance) * 100 : 0;
  ...
  const renderProgressBar = (metric: MetricData) => {
    ...
    const currentAsPercentOfPrevious = metric.previous > 0 ? (metric.current / metric.previous) * 100 : 100;
              ...
              <div className={`flex items-center space-x-1 ${
                ...
                >
                  {(() => {
                    ...
                    const progressPercent = metric.previous > 0 ? 
                      Math.round((metric.current / metric.previous) * 100) : 100;
